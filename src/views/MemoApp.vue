  ...
  <div class="memo-list">
    <ul class="memo-list__container">
      <li class="memo" v-for="item in memo" v-bind:key="item.id">
        ...
        <button @click="delete_memo(item.id)" class="memo__delete">削除</button>
      </li>
    </ul>
    <div class="add-memo-field">
      <input v-model="memo_text" class="add-memo-field__input" type="text" />
      <button v-on:click="add_memo" class="add-memo-field__button">追加</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    delete_memo: function (key) {
      for (let i = 0; i < this.memo.length; i++) {
        if (this.memo[i].id === key) {
